...
class MultiLayerPerceptronNumpy:
    ...
    def add_layer(self, layer_dim, activation, dropout_rate, input_dim):
        ...
        self.layers.append(SingleLayerPerceptronNumpy(layer_dim, input_dim, activation, dropout_rate))
    ...
    def batch_prediction(self, batch_input, batch_output, batch_size, l2_lambda):
        ...
        self.set_input_values(batch_input)
        batch_prediction = self.predict()

        batch_loss = self.loss_function.compute(batch_prediction, batch_output)
        batch_loss = self.l2_regularization_loss(batch_loss, l2_lambda)

        batch_error_vector = self.compute_raw_error(batch_prediction, batch_output)
    ...
    def update_weights_and_biases(self, raw_errors, learning_rate: float, l2_lambda: float):
        ...
        self.back_propagate()
        for layer in self.layers:
            layer.optimizer.update_weights_and_biases(learning_rate, l2_lambda)

    def l2_regularization_loss(self, loss, l2_lambda):
        l2_penalty = 0.5 * l2_lambda * np.sum([np.sum(layer.weights ** 2) for layer in self.layers])
    ...
    def compute_raw_error(self, prediction, target_output):
        return np.mean(prediction - target_output, axis=1, keepdims=True)